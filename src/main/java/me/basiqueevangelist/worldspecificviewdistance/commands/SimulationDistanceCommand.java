...
public final class SimulationDistanceCommand {
    ...
    public static int setWorldSimulationDistance(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        data.setLocalSimulationDistance(simDist);

        for (ServerPlayer player : l.players()) {
            player.connection.send(new ClientboundSetSimulationDistancePacket(simDist == 0 ? l.getServer().getPlayerList().getSimulationDistance() : simDist - 1));
        }

        l.getChunkSource().setSimulationDistance(simDist == 0 ? l.getServer().getPlayerList().getSimulationDistance(): simDist - 1);

        src.sendSuccess(() -> CommandUtils.getMessage(
    		"Set simulation distance of world %s to %d",
            CommandUtils.getRegistryId(l), simDist), true);
        return 1;
    }
    ...
    public static int getWorldSimulationDistance(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        WSVDSavedData data = WSVDSavedData.getFrom(l);
        int simDist = data.getLocalSimulationDistance();

        if (simDist != 0) {
            src.sendSuccess(() -> CommandUtils.getMessage(
                "Simulation distance of world %s is %d",
                CommandUtils.getRegistryId(l), simDist), false);
        }
        else {
            src.sendSuccess(() -> CommandUtils.getMessage("Simulation distance of world %s is unspecified (currently %d)",
                CommandUtils.getRegistryId(l), src.getServer().getPlayerList().getSimulationDistance() + 1), false);
        }
